...
describe User do
  ...
  it 'allows missing country and state' do
    expect { create(:user, country_code: nil, state_code: nil) }.not_to raise_error
  end

  it 'allows country without state' do
    expect { create(:user, country_code: 'ZA') }.not_to raise_error
  end

  it 'allows country with state' do
    expect { create(:user, country_code: 'US', state_code: 'IN') }.not_to raise_error
  end

  it 'denys stateless country with state' do
    expect { create(:user, country_code: 'ZA', state_code: 'IN') }
      .to raise_error(ActiveRecord::RecordInvalid)
  end

  it 'denys stateful country with missing state' do
    expect { create(:user, country_code: 'US', state_code: nil) }.to raise_error(ActiveRecord::RecordInvalid)
  end

  it 'denys stateful country with wrong state' do
    expect { create(:user, country_code: 'CA', state_code: 'IN') }
      .to raise_error(ActiveRecord::RecordInvalid)
  end
end
